...
async function init() {
  ...
  const enemyData = createEnemyTank(tankGlb.scene.clone());
...
function animate() {
  ...
  if (tankRigidBody) {
    ...
    tankRigidBody.setLinvel(
      { x: forward.x * currentSpeed, y: linVel.y, z: forward.z * currentSpeed },
      true
    );
  ...
  if (enemyRigidBody && !enemyDead) {
    ...
    enemyRigidBody.setLinvel(
      { x: eForward.x * eTargetSpeed, y: eLinVel.y, z: eForward.z * eTargetSpeed },
      true
    );
    ...
    if (hit || tooFar) {
      if (hit) {
        ...
        if (hitParent === enemyRigidBody && cb.owner === 'player' && !enemyDead) {
          enemyHealth = Math.max(0, enemyHealth - cannonDamage);
          if (enemyHealth <= 0) {
            enemyDead = true;
          ...
          playerHealth = Math.max(0, playerHealth - cannonDamage);
          if (playerHealth <= 0) {
            playerDead = true;
  ...
  if (wantsToFireLaser && mgHeat < mgHeatMax && !mgOverheated) {
    ...
    if (mgHeat < mgHeatMax) {
      ...
      if (hitParent === enemyRigidBody && !enemyDead && (now - lastLaserDamageTime) >= laserDamageInterval) {
        ...
        enemyHealth = Math.max(0, enemyHealth - laserDamagePerTick);
        if (enemyHealth <= 0) {
          enemyDead = true;
          explosionPieces.push(...explodeTank(enemyMesh, enemyRigidBody));
          enemyMesh = null;
